...
router = APIRouter(prefix="/api/v1/regulator/licenses", tags=["Regulator Licenses Admin"])
...
def extract_terms_reg_licenses(
    ...
    response_path = "./database/llm_raw_outputs/regulator/licenses"
    ...
    try:
        ...
        for file, file_name in zip(word_file, names_list):
            ...
            oss_path = f"cst/cst_extract_terms/regulator/licenses{blob_name}"
...
def generate_controls_reg_licenses(
    ...
    """
    DOCX_DIRECTORY = "./database/controls/regulator/licenses"
    results = []
    ...
    try:
        ...
        for entry in file_urls_json.files:
            ...
            # 6. Upload to OSS
            bucket = init_oss_bucket(ACCESS_KEY_ID, ACCESS_KEY_SECRET, ENDPOINT, BUCKET_NAME)
            oss_path = f"cst/cst_controls/controls_results/regulator/licenses/{name_file}.docx"
...
def create_rag_system(file_urls_json: FileURLsRag):
    ...
    DOCX_DIRECTORY = "./database/glasshub_files/regulator/licenses"
    path_upload = "cst/cst_rag/regulator/licenses"
    try:
        # ✅ Validate input
