...
router = APIRouter(prefix="/api/v1/enterprise/organization", tags=["Enterprise Organization Admin"])
...
def extract_terms_enterprise_organizations(
    ...
    response_path = "./database/llm_raw_outputs/enterprise/organization"
    ...
    try:
        ...
        for file, file_name in zip(word_file, names_list):
            ...
            oss_path = f"cst/cst_extract_terms/enterprise/organization{blob_name}"
...
def generate_controls_enterprise_organizations(
    ...
    """
    DOCX_DIRECTORY = "./database/controls/enterprise/organization"
    results = []
    ...
    try:
        ...
        for entry in file_urls_json.files:
            ...
            # 6. Upload to OSS
            bucket = init_oss_bucket(ACCESS_KEY_ID, ACCESS_KEY_SECRET, ENDPOINT, BUCKET_NAME)
            oss_path = f"cst/cst_controls/controls_results/enterprise/organization/{name_file}.docx"
...
def create_rag_system(file_urls_json: FileURLsRag):
    ...
    DOCX_DIRECTORY = "./database/glasshub_files/enterprise/organization"
    path_upload = "cst/cst_rag/enterprise/organization"
    try:
        # ✅ Validate input
